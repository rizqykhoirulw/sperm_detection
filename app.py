...
# ======================================================================
# Nilai default confidence & IOU threshold (dulu bisa diatur lewat sidebar,
# sekarang di-fix di kode supaya tampilan lebih sederhana untuk user)
# ======================================================================
...
# Fungsi heuristik: apakah gambar ini kemungkinan besar BUKAN citra sel sperma?
# Catatan: ini bukan classifier terpisah, hanya memanfaatkan confidence & jumlah
# deteksi dari model YOLO yang sudah ada. Kalau model tidak menemukan objek
# yang mirip sel sperma sama sekali (atau confidence-nya sangat rendah),
# kemungkinan besar gambar input memang bukan citra sel sperma.
def is_likely_not_sperm_image(detections):
    if len(detections) < MIN_VALID_DETECTIONS:
        return True
    max_conf = max(d["Confidence"] for d in detections)
    if max_conf < MIN_VALID_CONFIDENCE:
        return True
    return False
